...
// Конструктор
// -----------------------------------------------------
SharedMemory::SharedMemory(std::size_t paramSampleLength, std::string python_scripts_path)
    ...
    // 5) Импортируем Python-скрипт
    {
        ...
        sys.attr("path").attr("append")(std::string(python_scripts_path));
...
void SharedMemory::ensureClassRegistered() {
    if (!classRegistered_) {
        ...
        py::class_<SharedMemory>(moduleCppBindings_, "SharedMemory")
                .def(py::init<std::size_t, std::string>(), py::arg("sample_length"), py::arg("python_scripts_path"))
